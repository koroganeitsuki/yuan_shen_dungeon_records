...
const app = express();
// 定义HTTPS端口
// 注意：在Windows上监听1024以下端口需要管理员权限，建议使用非特权端口
const HTTPS_PORT = process.env.HTTPS_PORT || 443;

// 域名配置（用于自动生成证书）
const DOMAIN = process.env.DOMAIN || 'yjwz.de5.net';
const EMAIL = process.env.EMAIL || 'your-email@example.com'; // 用于Let's Encrypt注册
...
const generateSSLCertificate = () => {
    ...
    // 在Windows系统上直接使用自签名证书
    if (isWindows) {
        log('⚠️  Windows系统不支持直接安装acme.sh');
        log('🔄 将使用自签名证书');
        return generateSelfSignedCertificate();
    }
    
    // 在非Windows系统上尝试使用acme.sh
    try {
        execSync('acme.sh --version', { stdio: 'ignore' });
        log('✅ 已安装acme.sh，将使用acme.sh生成证书');
    } catch (error) {
        log('⚠️  未安装acme.sh，正在安装...');
        try {
            // 安装acme.sh
            execSync('curl https://get.acme.sh | sh -s email=' + EMAIL, { stdio: 'inherit' });
            // 添加到PATH
            process.env.PATH += ':/root/.acme.sh:/home/' + process.env.USER + '/.acme.sh';
            log('✅ acme.sh安装成功');
        } catch (installError) {
            log('❌ acme.sh安装失败：' + installError.message);
            log('⚠️  将使用自签名证书');
            return generateSelfSignedCertificate();
        }
    }
    ...
    try {
        log('🔄 正在使用acme.sh生成证书...');
        // 使用HTTP验证方式，需要80端口可用
        execSync('acme.sh --issue -d ' + DOMAIN + ' --webroot ' + path.join(__dirname, 'public'), { stdio: 'inherit' });
        
        // 安装证书到ssl目录
        execSync('acme.sh --install-cert -d ' + DOMAIN + ' --cert-file ' + certPath + ' --key-file ' + keyPath + ' --fullchain-file ' + path.join(sslDir, 'fullchain.crt'), { stdio: 'inherit' });
...
const generateSelfSignedCertificate = () => {
    ...
    const attrs = [
        ...
        { shortName: 'ST', value: 'Shanghai' },
        { name: 'localityName', value: 'Shanghai' },
        { name: 'organizationName', value: 'YuanShen' },
        { shortName: 'OU', value: 'Records' }
    ];
...
if (sslOptions.key && sslOptions.cert) {
    const httpsServer = https.createServer(sslOptions, app).listen(HTTPS_PORT, '::', () => {
        ...
        log(`=== HTTPS服务器启动信息 ===`);
        log(`HTTPS服务器启动成功，监听端口: ${HTTPS_PORT}`);
        log(`监听地址: ${address.address}`);
        log(`监听地址类型: ${address.family === 'IPv6' ? 'IPv6' : 'IPv4'}`);
        log(`=== HTTPS API访问地址 ===`);
        log(`健康检查 (IPv4): https://localhost:${HTTPS_PORT}/api/health`);
        log(`健康检查 (IPv6): https://[::1]:${HTTPS_PORT}/api/health`);
        log(`提交通关记录: POST https://localhost:${HTTPS_PORT}/api/submit-record`);
        log(`=== HTTPS网页访问地址 ===`);
        log(`首页 (IPv4): https://localhost:${HTTPS_PORT}/index.html`);
        log(`上传页 (IPv4): https://localhost:${HTTPS_PORT}/upload.html`);
        log(`排行榜 (IPv4): https://localhost:${HTTPS_PORT}/ranking.html`);
        log(`首页 (IPv6): https://[::1]:${HTTPS_PORT}/index.html`);
        log(`上传页 (IPv6): https://[::1]:${HTTPS_PORT}/upload.html`);
        log(`排行榜 (IPv6): https://[::1]:${HTTPS_PORT}/ranking.html`);
        log(`=== HTTPS公网访问地址 ===`);
        log(`首页: https://[240e:b8f:bc05:4300:fca3:271:e950:753]:${HTTPS_PORT}/index.html`);
        log(`上传页: https://[240e:b8f:bc05:4300:fca3:271:e950:753]:${HTTPS_PORT}/upload.html`);
        log(`排行榜: https://[240e:b8f:bc05:4300:fca3:271:e950:753]:${HTTPS_PORT}/ranking.html`);
        log(`健康检查: https://[240e:b8f:bc05:4300:fca3:271:e950:753]:${HTTPS_PORT}/api/health`);
        log(`提交通关记录: POST https://[240e:b8f:bc05:4300:fca3:271:e950:753]:${HTTPS_PORT}/api/submit-record`);
        log(`=== HTTPS服务器现在支持IPv4和IPv6访问 ===`);
    });
} else {
    log(`⚠️  HTTPS服务器启动失败：缺少SSL证书`);
}
